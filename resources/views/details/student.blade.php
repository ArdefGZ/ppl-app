        ...
        <div class="card-body">
            ...
            <table class="table table-bordered">
                <tbody>
                    ...
                    <tr>
                        <th scope="row">Wali</th>
                        <td>{{$mahasiswa->doswal->nama}}</td>
                    </tr>
                </tbody>
            </table>
            <hr />
            <div class="d-flex flex-wrap justify-content-center">
                @foreach($irs as $ir)
                @if($ir->NIM == $mahasiswa->NIM)
                <!-- Button trigger modal -->
                @if($mahasiswa->pkl->status_pkl == 'lulus' && $mahasiswa->pkl->smt_lulus == $ir->smst_aktif)
                <button type="button" class="btn btn-light custom-green-btn px-5 mx-2 mt-3 semester-button" data-toggle="modal" data-target="#semester{{$ir->smst_aktif}}">Semester {{$ir->smst_aktif}}</button>
                @elseif($mahasiswa->skripsi->status_skripsi == 'lulus' && $mahasiswa->skripsi->smt_lulus == $ir->smst_aktif)
                <button type="button" class="btn btn-light custom-yellow-btn px-5 mx-2 mt-3 semester-button" data-toggle="modal" data-target="#semester{{$ir->smst_aktif}}">Semester {{$ir->smst_aktif}}</button>
                @else
                <button type="button" class="btn btn-light px-5 mx-2 mt-3 semester-button" data-toggle="modal" data-target="#semester{{$ir->smst_aktif}}">Semester {{$ir->smst_aktif}}</button>
                @endif
                <style>
                    .custom-green-btn {
