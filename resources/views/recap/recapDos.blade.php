                        ...
                        <tbody>
                            <tr>
                                @for($i=2016;$i<=2023;$i++) <?php
                                                            $sudahLulusCount = 0;
                                                            $belumLulusCount = 0;
                                                            ?> @foreach($mahasiswa as $student) @if($student->angkatan == $i && $student->pkl && $student->pkl->status_pkl == 'lulus' && $student->NIP == $user->NIP)
                                    <?php $sudahLulusCount++; ?>
                                    @endif
                                    @if($student->angkatan == $i && (!$student->pkl || $student->pkl->status_pkl != 'lulus' && $student->NIP == $user->NIP))
                                    <?php $belumLulusCount++; ?>
                                    @endif
                                    @endforeach
                    ...
                    <table class="table" id="skripsi-table">
                        ...
                        <tbody>
                            @for($i=2016;$i<=2023;$i++) <?php
                                                        $sudahLulusCount = 0;
                                                        $belumLulusCount = 0;
                                                        ?> @foreach($mahasiswa as $student) @if($student->angkatan == $i && $student->skripsi && $student->skripsi->status_skripsi == 'lulus' && $student->NIP == $user->NIP)
                                <?php $sudahLulusCount++; ?>
                                @endif
                                @if($student->angkatan == $i && (!$student->skripsi || $student->skripsi->status_skripsi != 'lulus' && $student->NIP == $user->NIP))
                                <?php $belumLulusCount++; ?>
                                @endif
                                @endforeach
                                @if($sudahLulusCount == 0)
                                <td class="text-center">{{$sudahLulusCount}}</td>
                                @else
                                <td class="text-center"><a href="/dashboard/departemen/{{$user->ID_dep}}/recap/{{$i}}/skripsi/lulus">{{$sudahLulusCount}}</a></td>
                                @endif
                                @if($belumLulusCount == 0)
                                <td class="text-center">{{$belumLulusCount}}</td>
                                @else
                                <td class="text-center"><a href="/dashboard/departemen/{{$user->ID_dep}}/recap/{{$i}}/skripsi/belum">{{$belumLulusCount}}</a></td>
                                @endif
                                @endfor
                        </tbody>
                    </table>
            ...
            <div class="section" id="section3">
                <div class="table-responsive">
                    <table class="table" id="status-table">
                        <thead>
                            <tr>
                                <th class="text-center" colspan="24" scope="col">Rekap Status Mahasiswa</th>
                            </tr>
                            <tr>
                                <th class="text-center" colspan="24" scope="col">Angkatan</th>
                            </tr>
                            <tr>
                        ...
                        <tbody>
                            @for($i=2016;$i<=2023;$i++) <?php
                                                        ...
                                                        $undurDiriCount = 0;
                                                        $doCount = 0;
                                                        ?> @foreach($mahasiswa as $student) @if($student->angkatan == $i && $student->NIP == $user->NIP)
                                <?php
